...
void WorldserverCLI::TestActorPersistence(int argc, char **argv, argh::parser &cmd, std::string &description)
{
	...
	constexpr uint32_t actor_bot_id        = 400000001;
	constexpr uint32_t owner_character_id  = 401000001;
	...
	if (auto existing_profile = ActorProfilesRepository::FindByBotId(database, actor_bot_id)) {
		ActorStatusRepository::DeleteOne(database, existing_profile->actor_id);
		ActorProfilesRepository::DeleteOne(database, existing_profile->actor_id);
	}
	...
	Expect(
		ActorStatusRepository::FindByActorId(database, inserted_profile.actor_id).has_value(),
		"status lookup by actor_id should succeed"
	);

	ActorStatusRepository::DeleteOne(database, inserted_profile.actor_id);
	ActorProfilesRepository::DeleteOne(database, inserted_profile.actor_id);
