...
if [[ "$profile" != "preflight" ]]; then
  ...
  validate_args=(env "AKKSTACK_DIR=$AKKSTACK_STACK_DIR" "$repo_root/scripts/validate.sh" --stack "$AKKSTACK_STACK_ROLE")
