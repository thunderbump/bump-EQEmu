...
These are required delivery rules. Snapshot-based migration validation is available through the repository-owned [`migration-rehearsal` profile](../testing/database-migration-rehearsal.md). Its explicitly selected, checksummed fixture remains an operator-provided prerequisite. Never assume the shared validation database is disposable.
