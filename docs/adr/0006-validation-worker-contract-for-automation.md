...
Automated Bump EQEmu validation will use a portable **Validation Worker** contract instead of assuming that the automation process can access a sibling AkkStack path or the same Docker host. A validation request names a fetchable repository ref or commit, a validation profile such as `preflight`, `safe`, or `tier3-harness`, and an evidence directory; the worker fetches the ref into worker-owned storage, runs the project-specific validation profile against its persistent validation AkkStack/database, writes structured evidence, and returns a normal process exit code.
