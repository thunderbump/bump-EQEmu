...
The corpse path demonstrates both the valuable rules and the coupling: it establishes one active looter and player loot rights in [`Corpse::MakeLootRequestPackets`](https://github.com/thunderbump/bump-EQEmu/blob/bcf3473671d7f3967a0727e70b883c9254d97cd3/zone/corpse.cpp#L1219-L1497), and that list/request phase may also pay corpse coin into the Client wallet before any item is selected. It then reconstructs the instance and runs ordinary structural checks in [`Corpse::LootCorpseItem`](https://github.com/thunderbump/bump-EQEmu/blob/bcf3473671d7f3967a0727e70b883c9254d97cd3/zone/corpse.cpp#L1499-L1636). The same method invokes zone-controller, player, zone, and item `EVENT_LOOT` callbacks before source deletion; those callbacks may both veto and perform arbitrary quest side effects ([effectful callback block](https://github.com/thunderbump/bump-EQEmu/blob/bcf3473671d7f3967a0727e70b883c9254d97cd3/zone/corpse.cpp#L1637-L1737)). The item path then inserts into Client inventory before removing the corpse item in the [successful-loot tail](https://github.com/thunderbump/bump-EQEmu/blob/bcf3473671d7f3967a0727e70b883c9254d97cd3/zone/corpse.cpp#L1756-L1858). The new seam should extract or share the structural rules; it should not label arbitrary quest callbacks as preflight validation, reproduce the rules in an Actor-only implementation, or let `ClaimCorpseItem` implicitly claim coin.
