...
Every strategy requires a complete, versioned Bot Gear Value evaluation of a non-empty set of all relevant party Bots
before it may treat the absence of a positive upgrade as evidence for another disposition. Relevant Bot identities must
be valid, unique, and exactly covered by the evaluated set. Upgrade records must be structurally complete;
...
distribution, and rejected actions. These outcome metrics use item quantity units: `sell_through_units`,
`aged_unsold_units`, `ending_unit_stock_concentration_bps`, and per-Bot `equipped_units`; average sell-through days is
quantity-weighted. The plain `equip`, `hold`, `vendor`, and `offer_later` action totals count recommendations/listings,
not units. A separate custody/currency audit detects duplicate sources/transitions, invalid
...
Malformed outcome snapshots, including missing or non-integer/negative sale, buyer, settlement, or debit values, also
follow that expiry fallback and add stable `invalid_market_outcome` audit evidence instead of entering arithmetic.
