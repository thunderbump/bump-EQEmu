...
A host-local baseline has been captured and private inputs have been prepared. The latest refreshed-input run restored and seeded the snapshot, upgraded 9328 to 9335, and passed `tests:actor-events`, but its metadata-based JSON-constraint assertion failed because MariaDB 10.5 truncates the exposed check clauses. The behavioral checks above replace that assertion, but have not yet had an end-to-end run; idempotence and recovery were not reached, so this is not acceptance evidence. Generated SQL is not refreshed automatically when the repository preparer changes: the operator must rerun preparation for the exact Candidate, review the regenerated inputs, and install its selection file. Result evidence records the fixture-preparer commit plus exact snapshot and generated-SQL hashes so stale inputs remain distinguishable. The capture's source checkout is metadata, not proof that the archived installed binaries match. Compatibility, restore timing, updater idempotence, scenario behavior, and old-build recovery therefore remain unproven until fresh inputs are selected and the complete fixture passes.
