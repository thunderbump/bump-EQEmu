...
The validation worker writes optional `public-summary.json` beside its private result. It includes schema version 1, candidate `head`, `profile`, repository `status`, failed `step`, `diagnostic_codes`, and `timings_ms` containing nullable `validation` and `restore` durations. The projector reads structured worker/check/rehearsal records, never private log tails. It publishes only known profile, phase and assertion identifiers. Unknown assertion text is replaced with a generic code. A failed preparer reports `fixture_preparation`; missing baseline configuration reports `baseline_unavailable`. No paths, rows, credentials or snapshot/build identities enter this contract.

The nested rehearsal result retains a bounded `assertion_result` for the projector. It remains private; only allowlisted failed predicate names can reach the public summary. Summary generation is best effort and cannot replace the validation exit status. Its own failure is retained in `logs/public-summary.log`. AFK consumes a valid matching-head summary instead of the legacy log excerpts and otherwise falls back to its existing publication behavior. The host fixture state remains authoritative even when the repository report differs.
