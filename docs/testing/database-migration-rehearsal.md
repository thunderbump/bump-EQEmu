...
`./scripts/validate.sh --stack validation migration-rehearsal` is the repository-owned migration profile. It uses the existing EQEmu `world database:updates` updater and the validation AkkStack MariaDB process, but creates a uniquely named `afk_migration_*` database. It never imports into the persistent validation database. The wrapper rejects the gameplay role, the gameplay default path, an isolated name that does not have the reserved prefix, and a snapshot containing `USE`, `CREATE DATABASE`, or `DROP DATABASE` statements.
...
The profile reuses one MariaDB process and isolates by database name rather than by disposable server volume. It temporarily grants the validation `eqemu` user access only to the reserved database and revokes that grant during cleanup. It does not prove zero-downtime migration, universal reverse migrations, client rendering, cross-zone handoff, or production performance. Restore duration on local validation hardware is evidence for maintenance planning, not a production SLA. Snapshot acquisition, secret handling, and mounting the old binary remain trusted fixture-setup responsibilities.
